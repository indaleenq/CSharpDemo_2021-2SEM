...
namespace Implem1
{
    class Program
    {
        ...
        static string studentNumber = string.Empty;
        ...
        static bool ValidateStudentNumber()
        {
            ...
            return SqlData.ValidateIfStudentExists(studentNumber);
            //return userInput.Equals(studentNumber) ? true : false; //ternary operators
        }

        static string GetCurrentPoints()
        {
            return $"Total points as of {DateTime.Now}: {SqlData.GetPointsByStudent(studentNumber)}";
        }
